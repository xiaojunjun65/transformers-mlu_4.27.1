...
class Distiller:
    def __init__(
        ...
        if self.mlm:
            ...
            self.pred_probs = self.pred_probs.to(f"mlu:{params.local_rank}") if params.n_gpu > 0 else self.pred_probs
            self.token_probs = token_probs.to(f"mlu:{params.local_rank}") if params.n_gpu > 0 else token_probs
            if self.fp16:
        ...
        for _ in range(self.params.n_epoch):
            ...
            for batch in iter_bar:
                if self.params.n_gpu > 0:
                    batch = tuple(t.to(f"mlu:{self.params.local_rank}") for t in batch)
