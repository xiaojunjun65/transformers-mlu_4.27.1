...
import datasets
import numpy as np
import pymlu.autoinit  # noqa: F401
import pymlu.driver as mlu
...
def model_infer(inputs, context, d_inputs, h_output0, h_output1, d_output0, d_output1, stream):
    ...
    # Copy inputs
    mlu.memcpy_htod_async(d_inputs[0], input_ids.ravel(), stream)
    mlu.memcpy_htod_async(d_inputs[1], attention_mask.ravel(), stream)
    mlu.memcpy_htod_async(d_inputs[2], token_type_ids.ravel(), stream)
    # start time
    ...
    mlu.memcpy_dtoh_async(h_output0, d_output0, stream)
    mlu.memcpy_dtoh_async(h_output1, d_output1, stream)
...
with open(engine_name, "rb") as f, trt.Runtime(TRT_LOGGER) as runtime, runtime.deserialize_mlu_engine(
    f.read()
    ...
    d_inputs = [mlu.mem_alloc(binding_nbytes(binding)) for binding in engine if engine.binding_is_input(binding)]

    # Allocate output buffer
    h_output0 = mlu.pagelocked_empty(tuple(context.get_binding_shape(3)), dtype=np.float32)
    h_output1 = mlu.pagelocked_empty(tuple(context.get_binding_shape(4)), dtype=np.float32)
    d_output0 = mlu.mem_alloc(h_output0.nbytes)
    d_output1 = mlu.mem_alloc(h_output1.nbytes)
