...
def main():
    ...
    if args.local_rank == -1 or args.no_mlu:
        args.device = torch.device("mlu" if torch.mlu.is_available() and not args.no_mlu else "cpu")
        args.n_gpu = 0 if args.no_mlu else torch.mlu.device_count()
    else:
        torch.mlu.set_device(args.local_rank)
        args.device = torch.device("mlu", args.local_rank)
        args.n_gpu = 1
        torch.distributed.init_process_group(backend="cncl")  # Initializes the distributed backend
