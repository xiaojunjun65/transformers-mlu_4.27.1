...
class TrainerIntegrationTest(TestCasePlus, TrainerIntegrationCommon):
    ...
    def test_number_of_steps_in_training_with_ipex(self):
        for mix_bf16 in [True, False]:
            # Regular training has n_epochs * len(train_dl) steps
            trainer = get_regression_trainer(learning_rate=0.1, use_ipex=True, bf16=mix_bf16, no_mlu=True)
            train_output = trainer.train()
            self.assertEqual(train_output.global_step, self.n_epochs * 64 / trainer.args.train_batch_size)

            # Check passing num_train_epochs works (and a float version too):
            trainer = get_regression_trainer(
                learning_rate=0.1, num_train_epochs=1.5, use_ipex=True, bf16=mix_bf16, no_mlu=True
            )
            train_output = trainer.train()
            ...
            # If we pass a max_steps, num_train_epochs is ignored
            trainer = get_regression_trainer(
                learning_rate=0.1, max_steps=10, use_ipex=True, bf16=mix_bf16, no_mlu=True
            )
            train_output = trainer.train()
            self.assertEqual(train_output.global_step, 10)
    ...
    def test_train_and_eval_dataloaders(self):
        n_gpu = max(1, torch.mlu.device_count())
    ...
    def test_evaluate_with_ipex(self):
        for mix_bf16 in [True, False]:
            trainer = get_regression_trainer(
                a=1.5, b=2.5, use_ipex=True, compute_metrics=AlmostAccuracy(), bf16=mix_bf16, no_mlu=True
            )
            results = trainer.evaluate()
    ...
    def test_predict_with_ipex(self):
        for mix_bf16 in [True, False]:
            trainer = get_regression_trainer(a=1.5, b=2.5, use_ipex=True, bf16=mix_bf16, no_mlu=True)
            preds = trainer.predict(trainer.eval_dataset).predictions
            x = trainer.eval_dataset.x
            self.assertTrue(np.allclose(preds, 1.5 * x + 2.5))

            # With a number of elements not a round multiple of the batch size
            trainer = get_regression_trainer(a=1.5, b=2.5, eval_len=66, use_ipex=True, bf16=mix_bf16, no_mlu=True)
            preds = trainer.predict(trainer.eval_dataset).predictions
            x = trainer.eval_dataset.x
            self.assertTrue(np.allclose(preds, 1.5 * x + 2.5))
            ...
            # With more than one output of the model
            trainer = get_regression_trainer(
                a=1.5, b=2.5, double_output=True, use_ipex=True, bf16=mix_bf16, no_mlu=True
            )
            preds = trainer.predict(trainer.eval_dataset).predictions
            x = trainer.eval_dataset.x
            self.assertEqual(len(preds), 2)
    ...
    def test_resume_training_with_randomness(self):
        ...
        random_torch = not torch.mlu.is_available() or torch.mlu.device_count() <= 1
    ...
    def test_trainer_eval_mrpc(self):
        ...
        training_args = TrainingArguments(output_dir="./examples", no_mlu=True)
    ...
    def test_torchdynamo_memory(self):
        ...
        a = torch.ones(1024, 1024, device="mlu", requires_grad=True)
        a.grad = None
        ...
        orig_loss = trainer.training_step(mod, {"x": a})
        orig_peak_mem = torch.mlu.max_memory_allocated()
        torchdynamo.reset()
        del trainer
        ...
        loss = trainer.training_step(mod, {"x": a})
        peak_mem = torch.mlu.max_memory_allocated()
        torchdynamo.reset()
        del trainer
