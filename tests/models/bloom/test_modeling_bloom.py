...
class BloomModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    def test_simple_generation(self):
        ...
        # Please see: https://pytorch.org/docs/stable/notes/mlu.html#reduced-precision-reduction-in-fp16-gemms
        ...
        model = BloomForCausalLM.from_pretrained(path_560m, use_cache=True, revision="gs555750").mlu()
        model = model.eval()
        ...
        greedy_output = model.generate(input_ids.mlu(), max_length=50)
    ...
    def test_batch_generation(self):
        ...
        greedy_output = model.generate(
            input_ids["input_ids"].mlu(), attention_mask=input_ids["attention_mask"], max_length=50, do_sample=False
        )
    ...
    def test_batch_generation_padd(self):
        ...
        greedy_output_without_pad = model.generate(input_ids_without_pad.mlu(), max_length=50, do_sample=False)
...
class BloomEmbeddingTest(unittest.TestCase):
    ...
    @require_torch
    @unittest.skipIf(
        is_torch_less_than_1_9, reason="Test failed with torch < 1.9 (`min_mlu` not implemented for `BFloat16`)"
    )
