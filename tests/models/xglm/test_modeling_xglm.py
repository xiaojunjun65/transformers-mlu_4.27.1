...
class XGLMModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_batched_nan_fp16(self):
        ...
        model = XGLMForCausalLM.from_pretrained(model_name, torch_dtype=torch.float16, use_cache=True).mlu()
        model = model.eval()

        batch = tokenizer(["Who are you?", "Joe Biden is the president of"], padding=True, return_tensors="pt")

        input_ids = batch["input_ids"].mlu()
        attention_mask = batch["attention_mask"].mlu()
