...
def distributed_broadcast_scalars(
    ...
    device: Optional[torch.device] = torch.device("mlu"),
) -> torch.Tensor:
    try:
...
def log_metrics(self, split, metrics):
    ...
    **Understanding the reports:**
    ...
    The GPU allocated and peak memory reporting is done with `torch.mlu.memory_allocated()` and
    `torch.mlu.max_memory_allocated()`. This metric reports only "deltas" for pytorch-specific allocations, as
    `torch.mlu` memory management system doesn't track any memory allocated outside of pytorch. For example, the very
    first mlu call typically loads CUDA kernels, which may take from 0.5 to 2GB of GPU memory.
    ...
    `torch.mlu.max_memory_allocated` is a single counter, so if it gets reset by a nested eval call, `train`'s tracker
    ...
    `torch.mlu.reset_peak_memory_stats`, the gpu peak memory stats could be invalid. And the [`Trainer`] will disrupt
    the normal behavior of any such tools that rely on calling `torch.mlu.reset_peak_memory_stats` themselves.
