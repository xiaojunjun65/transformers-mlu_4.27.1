...
class TrainingArguments:
    ...
    no_mlu: bool = field(default=False, metadata={"help": "Do not use CUDA even when it is available"})
    ...
    def _setup_devices(self) -> "torch.device":
        ...
        if self.no_mlu:
            ...
            if self.use_mps_device:
                ...
                # GPUs available in the environment, so `CUDA_VISIBLE_DEVICES=1,2` with `mlu:0`
            ...
            if not torch.distributed.is_initialized():
                torch.distributed.init_process_group(backend="cncl", timeout=self.ddp_timeout_delta)
            device = torch.device("mlu", self.local_rank)
            self._n_gpu = 1
