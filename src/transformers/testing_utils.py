...
def require_torch_non_multi_gpu(test_case):
    ...
    return unittest.skipUnless(torch.mlu.device_count() < 2, "test requires 0 or 1 GPU")(test_case)
...
def require_torch_up_to_2_gpus(test_case):
    ...
    return unittest.skipUnless(torch.mlu.device_count() < 3, "test requires 0 or 1 or 2 GPUs")(test_case)
...
if is_torch_available():
    ...
    torch_device = "mlu" if torch.mlu.is_available() else "cpu"
else:
    torch_device = None
...
def require_torch_gpu(test_case):
    ...
    return unittest.skipUnless(torch_device == "mlu", "test requires CUDA")(test_case)


def require_torch_bf16_gpu(test_case):
    """Decorator marking a test that requires torch>=1.10, using Ampere GPU or newer arch with mlu>=11.0"""
    return unittest.skipUnless(
        is_torch_bf16_gpu_available(),
        "test requires torch>=1.10, using Ampere GPU or newer arch with mlu>=11.0",
    )(test_case)
...
def require_torch_tf32(test_case):
    """Decorator marking a test that requires Ampere or a newer GPU arch, mlu>=11 and torch>=1.7."""
    return unittest.skipUnless(
        is_torch_tf32_available(), "test requires Ampere or a newer GPU arch, mlu>=11 and torch>=1.7"
    )(test_case)
