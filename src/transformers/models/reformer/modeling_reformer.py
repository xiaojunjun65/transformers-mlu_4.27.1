...
class ReformerLayer(nn.Module):
    ...
    def _init_attention_seed(self):
        ...
        # randomize seeds
        # use mlu generator if available
        if hasattr(torch.mlu, "default_generators") and len(torch.mlu.default_generators) > 0:
            # GPU
            device_idx = torch.mlu.current_device()
            self.attention_seed = torch.mlu.default_generators[device_idx].seed()
        else:
            # CPU
    ...
    def _init_feed_forward_seed(self):
        ...
        if hasattr(torch.mlu, "default_generators") and len(torch.mlu.default_generators) > 0:
            # GPU
            device_idx = torch.mlu.current_device()
            self.feed_forward_seed = torch.mlu.default_generators[device_idx].seed()
        else:
            # CPU
