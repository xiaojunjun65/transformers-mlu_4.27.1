...
def is_torch_bf16_gpu_available():
    ...
    # some bits come from https://github.com/pytorch/pytorch/blob/2289a12f21c54da93bf5d696e3f9aea83dd9c10d/torch/testing/_internal/common_mlu.py#L51
    ...
    if torch.mlu.is_available() and torch.version.mlu is not None:
        if torch.mlu.get_device_properties(torch.mlu.current_device()).major < 8:
            return False
        if int(torch.version.mlu.split(".")[0]) < 11:
            return False
        if not hasattr(torch.mlu.amp, "autocast"):
            return False
    else:
        return False
...
def is_torch_tf32_available():
    ...
    if not torch.mlu.is_available() or torch.version.mlu is None:
        return False
    if torch.mlu.get_device_properties(torch.mlu.current_device()).major < 8:
        return False
    if int(torch.version.mlu.split(".")[0]) < 11:
