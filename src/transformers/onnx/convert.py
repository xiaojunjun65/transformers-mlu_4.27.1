...
def export_pytorch(
    ...
    Args:
        ...
        device (`str`, *optional*, defaults to `cpu`):
            The device on which the ONNX model will be exported. Either `cpu` or `mlu`.
    ...
    if issubclass(type(model), PreTrainedModel):
        ...
        with torch.no_grad():
            ...
            if device.type == "mlu" and torch.mlu.is_available():
                model.to(device)
                model_inputs_device = {}
...
def export(
    ...
    Args:
        ...
        output (`Path`):
            Directory to store the exported ONNX model.
        device (`str`, *optional*, defaults to `cpu`):
            The device on which the ONNX model will be exported. Either `cpu` or `mlu`. Only PyTorch is supported for
    ...
    if is_tf_available() and isinstance(model, TFPreTrainedModel) and device == "mlu":
